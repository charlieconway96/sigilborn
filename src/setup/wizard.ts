...
export async function runSetupWizard(): Promise<AutomatonConfig> {
  ...
  console.log(chalk.white("  First-run setup. Let's bring your Sigil to life.\n"));
  ...
  console.log(chalk.dim(`  Secret key stored at: ${getAutomatonDir()}/wallet.json\n`));

  // ─── 2. Provision API key ─────────────────────────────────────
  console.log(chalk.cyan("  [2/6] Provisioning Conway API key (Solana signature)..."));
  let apiKey = "";
  try {
  ...
  const name = await promptRequired("What do you want to name your Sigil?");
  console.log(chalk.green(`  Name: ${name}\n`));

  const genesisPrompt = await promptMultiline("Enter the genesis prompt (system prompt) for your Sigil.");
  console.log(chalk.green(`  Genesis prompt set (${genesisPrompt.length} chars)\n`));

  const creatorAddress = await promptAddress("Your Solana wallet address");
...
function showFundingPanel(address: string): void {
  ...
  console.log(chalk.cyan(`  │${pad("  Fund your Sigil", w)}│`));
  console.log(chalk.cyan(`  │${" ".repeat(w)}│`));
  ...
  console.log(chalk.cyan(`  │${" ".repeat(w)}│`));
  console.log(chalk.cyan(`  │${pad("  2. Send SOL directly to the address above", w)}│`));
  console.log(chalk.cyan(`  │${" ".repeat(w)}│`));
  console.log(chalk.cyan(`  │${pad("  3. Fund via Conway Cloud dashboard", w)}│`));
  console.log(chalk.cyan(`  │${pad("     https://app.conway.tech", w)}│`));
  console.log(chalk.cyan(`  │${" ".repeat(w)}│`));
  console.log(chalk.cyan(`  │${pad("  The Sigil will start now. Fund it anytime —", w)}│`));
